...
class ActionDimension:
    def __init__(self,name,default_value,step,rrange,frozen=False,model={"name":"normal","param":0.5}):
        ...
        # 最多保留一位小数
        self.sample_box = [
            round(self.rrange[0] + idx * step, 1) if round(self.rrange[0] + idx * step, 1) % 1 != 0 else int(round(self.rrange[0] + idx * step, 0))
            for idx in range(int(self.scale))
        ]
        
        logger.info(f"name : {self.name} , default_value : {self.default_value} , step : {self.step} , rrange : {self.rrange}")
