...
def run_mcpat(config):
    try:
        if os.path.exists(f"./out/simout/3.txt"):
            # Gem5ToMcPAT
            subprocess.run(["python3", "./cMcPAT/Scripts/GEM5ToMcPAT.py", f"./out/simout/3.txt", f"./out/simout/config.json", 
                            f"./cMcPAT/mcpat/ProcessorDescriptionFiles/x86_AtomicSimpleCPU_template_core_{config['core']}.xml", "-o", f"./out/simout/test.xml"])
            # startMcPAT
            with open(f"./out/simout/test2.log", "w") as file_output:
                subprocess.run(["./cMcPAT/mcpat/mcpat", "-infile", f"./out/simout/test.xml", "-print_level", "5"], stdout=file_output)
            # get metrics
...
def evaluation(status):
    ...
    if row is not None:
        ...
        if metrics:
            ...
            logger.info(f"latency: {metrics['latency']} sec , area: {metrics['area']} mm^2 , energy: {metrics['energy']} mJ , power: {metrics['power']} W")
...
def read_csv_must(file_path):
    ...
    if os.path.exists(file_path):
        ...
        columns = ['core', 'l1i_size', 'l1d_size', 'l2_size', 'l1d_assoc', 'l1i_assoc', 'l2_assoc', 'sys_clock', 'latency', 'area', 'power']
